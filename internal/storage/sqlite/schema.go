// Package sqlite provides SQLite implementations of storage interfaces.
package sqlite

// Schema contains the SQL statements to create the database schema.
// This schema supports v2.0 async enrichment with status tracking.
const Schema = `
...
CREATE TABLE IF NOT EXISTS memories (
    ...
    -- Lifecycle state fields (migration 000002)
    state TEXT,
    state_updated_at TIMESTAMP,

    -- Provenance tracking fields (migration 000002)
    created_by TEXT,
    session_id TEXT,
    source_context TEXT, -- JSON

    -- Quality signal fields (migration 000002)
    access_count INTEGER NOT NULL DEFAULT 0,
    last_accessed_at TIMESTAMP,
    decay_score REAL NOT NULL DEFAULT 1.0,
    decay_updated_at TIMESTAMP,

    -- Classification and enrichment tracking fields (migration 000004)
    memory_type TEXT,
    classification TEXT,
    classification_status TEXT NOT NULL DEFAULT 'pending',
    summarization_status TEXT NOT NULL DEFAULT 'pending',

    -- Summarization output fields (migration 000008)
    summary TEXT,
    key_points TEXT,  -- stored as JSON array

    -- Soft delete (migration 000009)
    deleted_at TIMESTAMP,

    -- Content hash for deduplication (migration 000010)
    content_hash TEXT,

    -- Evolution chain (migration 000011)
...
-- User settings (e.g., user_name) are stored here to survive restarts.
...
-- Trigger to auto-update updated_at on settings modification
...
-- that were not in the allowed list. Used to surface commonly-skipped types
-- in the UI so operators can decide whether to promote them to custom types.
